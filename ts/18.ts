...
(async () => {
  ...
  const numbers = input
    ...
    .map((input) => parseNode(input)) as ParentNode[];
  const result = numbers.reduce<ParentNode>((acc, node, i) => {
    if (i === 0) {
      return acc;
    }
    const result = add(acc, node);
    return result;
  }, numbers[0]);
  const magnitude = getMagnitude(result);
  console.log('Result magnitude:', magnitude);
})();
